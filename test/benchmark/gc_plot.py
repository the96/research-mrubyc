...
for file_path in sys.argv:
  ...
  head=head_pat.search(lines.pop(0))
  if head:
    ...
    raise ParseErrorException("Header infomation is not found.")
  ...
  gc_flag = MARKSWEEP if type(gc_result) is MarkSweepGCResult else REFCOUNT
  for line in lines:

...
results[vm_names[1]] = swap(MS2_PER_RC_64, labels, test_list)
results[vm_names[2]] = swap(MS1_PER_RC_32, labels, test_list)
results[vm_names[3]] = swap(MS2_PER_RC_32, labels, test_list)
  ...
  for idx in range(0, len(values)):
    if values[idx] > 220:
      plt.text(
               idx + 1.15 + width * 2,
              #  xpoints[idx],
               fill_y_max - 36 - 12 * group_num,"{:7.3f}%".format(values[idx]),
               verticalalignment='center',
               horizontalalignment='left',
              )
      plt.hlines([fill_y_max - 36 - 12 * group_num], xpoints[idx], idx + 1.1 + width * 2, linestyle="solid", color="dimgrey")
  return xmax
...
plt.plot([0, xmax + width*1],[100,100], "red", linestyle='dashed', label="RC")

plt.legend(fontsize=12, loc='upper left')
plt.ylabel(u"ヒープサイズの比[%]", fontproperties=font_prop)
plt.xticks(xticks, test_list, rotation=12, fontsize=12)
plt.tight_layout()

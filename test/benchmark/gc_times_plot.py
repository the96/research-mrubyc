...
class GCTime:
  def __init__(self, gc_type):
    self.gc_times = {}
    self.gc_type = gc_type
    self.test_times = {}

  def new_test(self, heap_size):
    if not heap_size in self.test_times:
      self.test_times[heap_size] = 0
    self.test_times[heap_size] += 1
  
  def inc_gc_times(self, mark_time, sweep_time, heap_size):
    if not heap_size in self.gc_times:
      self.gc_times[heap_size] = 0
    self.gc_times[heap_size] += 1
  
  def get_key_value(self):
    keys = sorted(self.gc_times.keys())
    values = []
    for key in keys:
      values.append(int(self.gc_times[key] / self.test_times[key]))
    return (keys, values)
  ...
  for line in lines:
    ...
    if res:
      ...
      if vm_name in gc_bench.keys():
        if vm_name != "rc-gc":
          current_result = GCTime(TYPE_MS)
          gc_flag = MARKSWEEP
        else:
          current_result = GCTime(TYPE_RC)
          return
    ...
    if success:
      ...
      if gc_flag == NOGCBENCH:
        current_result.insert(heap_size, total_time)
      else:
        current_result.new_test(heap_size)
    ...
    if gc_flag == MARKSWEEP:
      ...
      if gc_time_match:
        ...
        current_result.inc_gc_times(mark_time, sweep_time, heap_size)
...
for vm_env in results.keys():
  ...
  for test_name in results[vm_env].keys():
    for vm_name in results[vm_env][test_name].keys():
      if "rc" in vm_name:
        continue
      result = results[vm_env][test_name][vm_name]
      if PLOT_MODE == ON:
        out_dir = "new_graph/" + vm_env + "/gc_times/"
        os.makedirs(out_dir, exist_ok=True)
        pdf_path = out_dir + test_name + "_" + vm_name + ".pdf"
      ...
      if "_m32" in test_name:
        _test_name =  test_name.replace('_m32', '(32bit)')
      else:
        _test_name = test_name + "(64bit)"

      # ラベルの配列を作成
      x_data, y_data = result.get_key_value()
      if len(y_data) > 0:
        print(test_name + ": " + str(y_data[0]))
      else:
        continue
      if PLOT_MODE == OFF:
        for i in range(0, len(y_data)):
          print(str(x_data[i]) + " heap " + str(x_data[i] / x_data[0]) + " times " + str(y_data[i] / y_data[0]))
        continue
      step = None
      y_max = None
      xticks = []
      ...
      ax.set_ylabel(u"GC回数", fontproperties=font_prop)
      ax.grid()
      plt.tight_layout()
      pdf.savefig()
      pdf.close()
